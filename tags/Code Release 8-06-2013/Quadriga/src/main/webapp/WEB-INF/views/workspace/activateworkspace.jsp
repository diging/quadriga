...
$(function() {
	...
	$("input[name='activatews']").button().click(function(event){
		if(!$("input[name='wschecked']").is(":checked")) {
			$.alert("Select record to activate", "Oops !!!");
			event.preventDefault();
			return;
		}
	});
...
<article class="is-page-content">
	<form:form modelAttribute="workspace" method="POST"
		action="${pageContext.servletContext.contextPath}/auth/workbench/${wsprojectid}/activateworkspace" id="activatewsform">
		<c:if test="${not empty workspaceList}">
			...
			<input class="command" type="submit" value='Activate' name="activatews">
