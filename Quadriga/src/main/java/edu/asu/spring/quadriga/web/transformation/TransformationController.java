...
public class TransformationController {
    ...
    public String transformTest(@RequestParam("project") String project,
            @RequestParam("transformation") String transformationNameAndID,
        ...
        String[] transformationAndDirectory = transformationNameAndID.split(",");
        String directoryName = null;

        if (transformationAndDirectory.length > 1)
            directoryName = transformationAndDirectory[1];

        ArrayList<String> fileNames = transformationManager.retrieveTransformationFilePaths(directoryName);
        String absolutePatternFilePath = fileNames.get(0);
        String absoluteMappingFilePath = fileNames.get(1);

        ITransformation transform = new Transformation();
        transform.setPatternFilePath(absolutePatternFilePath);
        transform.setTransformationFilePath(absoluteMappingFilePath);
