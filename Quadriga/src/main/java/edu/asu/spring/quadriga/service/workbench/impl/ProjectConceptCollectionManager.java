...
public class ProjectConceptCollectionManager implements IProjectConceptCollectionManager {
     ...
     * @param -
     *            conceptCollectionId - concept collection id
     * @param -
     *            userId - logged in user name.
    ...
    @Override
    @Transactional
    public List<IConceptCollection> getConceptCollections(String projectId) throws QuadrigaStorageException {

        ProjectDTO projectDTO = projManager.getDTO(projectId);
        IProject project = projDeepMapper.getProject(projectDTO);
        return projCCShallowMapper.getConceptCollections(project, projectDTO.getProjectConceptCollectionDTOList());
    }
