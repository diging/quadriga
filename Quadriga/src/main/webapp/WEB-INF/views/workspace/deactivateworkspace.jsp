...
<article class="is-page-content">
	<form:form modelAttribute="workspaceform" method="POST"
		action="${pageContext.servletContext.contextPath}/auth/workbench/${workspaceid}/deactivateworkspace?projectid=${wsprojectid}"
		id="deactivatewsform">
		<c:choose>
			<c:when test="${success == '0'}">
				<c:if test="${not empty workspaceform.workspaceList}">
					...
					<input class="command" type="submit" value='Deactivate'
						name="deactivatews">
					...
					<table style="width: 100%" class="display dataTable"
						...
						<tbody>
							<c:forEach var="workspace" items="${workspaceform.workspaceList}"
								varStatus="status">
								<tr>
									<td><form:checkbox
											path="workspaceList[${status.index}].id"
											value="${workspace.id}" /></td>
									<td><font size="3"> <form:label
												path="workspaceList[${status.index}].name">
												<c:out value="${workspace.name}"></c:out>
											</form:label>
									</font></td>
									<td><font size="3"> <form:label
												path="workspaceList[${status.index}].description">
			...
			<c:when test="${success == '1'}">
				<span class="byline">Successfully deactivated selected
					workspace</span>
				<ul>
					<li><input type="button" onClick="submitClick(this.id);"
						value='Okay' name="Back"></li>
			...
			<c:when test="${success == '2'}">
				<span class="byline">Do you want to deactivate this
					workspace?</span>
				<ul>
					<li><input class="command" type="submit" value='Deactivate'
						name="deactivatews"></li>
					<li><input type="button" onClick="submitClick(this.id);"
