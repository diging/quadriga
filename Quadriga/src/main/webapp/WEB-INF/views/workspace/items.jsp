		...
		<c:choose>
			<c:when test="${not empty itemList}">
				...
				<c:forEach var="item" items="${itemList}">
					<span style="font-weight: bold"><a href="/quadriga/auth/workbench/workspace/${workspaceId}/community/collection/item?collectionId=${collectionId}&itemId=${item.id}" style="color:#707070">${item.name}</a></span>
					<br />
				</c:forEach>
			</c:when>
