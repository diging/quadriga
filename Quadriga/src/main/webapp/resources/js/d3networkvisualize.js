...
function d3init(graph, networkId, path, type) {
	...
	if (type == "force") {
		...
		displayAllAnnotationsNew(networkId, path);
	...
	var node = gnodes.append('circle').attr("class", "node").attr("r", 10)
			...
			.on("click", function(d) {
				...
				display_annotations(d, path, networkId);
	...
	function displayAllAnnotations() {
		...
		$.ajax({
			...
			success : function(data) {
				...
				$.each(data.text, function(key, value) {
					// content += ++cnt
					// +'.<li>'+value.name+'</li>'+value.text+'</li>';
					output += "<li>" + ++cnt + ")" + " " + value.name + " "
							+ "-->" + " " + value.text + "</li>";
				});
				output += "</ol>";
	...
	function rightClick(d) {
		...
		if (d.group == 1) {
			...
			$("textarea[name='annotText'").attr('id', text1ID);
			$("#nodeid").attr("value", d.id);
			$("#nodename").attr("value", d.name);
			$('#annotationModal').modal('show');
		}
		// Annotate on node
		else {
			$("textarea[name='annotTextRel'").attr('id', text1ID);
			$("#nodenameRel").attr("value", d.name);
			$("#nodeidRel").attr("value", d.id);
			$('#annotationModalPredicate').modal('show');
		}
	...
	function add_annotationstolink(d) {
		...
		// Get annotation URL
		var getAnnotationUrl = path + "/auth/editing/getAnnotationOfEdge/"
				+ networkId;

		// Creating popup html content to facilitate adding annotation
		var html = "<div id='" + popupId + "' title='Annotation' >" + // style='display:
																		// none'
		"<form id='annot_edge_form' action=" + path
				+ "/auth/editing/saveAnnotationToEdge/";
		html += networkId + " method='GET' >";
		html += "<textarea name='annotText' id='" + text1ID
				+ "' cols='15' rows='15' ></textarea>";
		...
		$('#annot_submit_edge').click(
				function(event) {
					...
					var objecttype = "edge";
					var annottext = $('#' + text1ID).val();
					var url = path + "/auth/editing/saveAnnotationToEdge/"
							+ networkId;
					var input = "annottext=" + annottext + "&sourceid="
	...
	function display_annotations_of_edge(source, target) {
		...
		$.ajax({
			...
			type : "GET",
			data : "sourceid=" + source.id + "targetid=" + target.id + "&type="
					+ objecttypetype,
			dataType : 'json',
			success : function(data) {
				var cnt = 0;
